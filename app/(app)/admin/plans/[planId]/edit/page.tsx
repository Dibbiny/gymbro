...
export default async function AdminEditPlanPage({ params }: Props) {
  ...
  if (!plan) notFound();
  // Admin can edit any plan; only block non-owners who aren't admin
  if (plan.creatorId !== session!.user.id && session!.user.role !== "ADMIN") notFound();
  ...
  return (
    <div className="space-y-4">
      ...
      <PlanBuilder planId={planId} initialData={initialData} redirectTo={`/admin/plans/${planId}`} />
    </div>
  );
}
